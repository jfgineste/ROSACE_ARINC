...
RETURN_CODE_TYPE ret_process, ret_switch_mode; 
  ...
  CREATE_SAMPLING_PORT("WDELTAEC", PORT_SIZE, SOURCE, 1 * SECOND,&WDELTAEC,&ret);
  CREATE_SAMPLING_PORT("WDELTATHC", PORT_SIZE, SOURCE, 1 * SECOND,&WDELTATHC,&ret);

  printf("[P3] Bilan create output ports: DELTA_EC=%d DELTA_THC=%d\n", (int) WDELTAEC, (int) WDELTATHC);

  CREATE_SAMPLING_PORT("RHF", PORT_SIZE, DESTINATION, 1 * SECOND,&RHF,&ret);
  CREATE_SAMPLING_PORT("RAZF", PORT_SIZE, DESTINATION, 1 * SECOND,&RAZF,&ret);
  CREATE_SAMPLING_PORT("RVZF", PORT_SIZE, DESTINATION, 1 * SECOND,&RVZF,&ret);
  CREATE_SAMPLING_PORT("RQF", PORT_SIZE, DESTINATION, 1 * SECOND,&RQF,&ret);
  CREATE_SAMPLING_PORT("RVAF", PORT_SIZE, DESTINATION, 1 * SECOND,&RVAF,&ret);
