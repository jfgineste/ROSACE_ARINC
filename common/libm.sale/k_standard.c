...
static const double kstandard_zero = 0.0;	/* used as const */
...
#define POK_ERRNO_HUGE_VAL inf
	...
	switch(type) {
		...
		if (_LIB_VERSION == _SVID_) {
		  exc.retval = POK_ERRNO_HUGE_VAL;
		  y *= 0.5;
		  if(x<kstandard_zero&&rint(y)!=y) exc.retval = -POK_ERRNO_HUGE_VAL;
		} else {
		  exc.retval = POK_ERRNO_HUGE_VAL;
		  y *= 0.5;
		  if(x<kstandard_zero&&rint(y)!=y) exc.retval = -POK_ERRNO_HUGE_VAL;
		...
		if (_LIB_VERSION == _SVID_)
		    ...
		    exc.retval = kstandard_zero/kstandard_zero;	/* X/Open allow NaN */
		if (_LIB_VERSION == _POSIX_)
		   errno = POK_ERRNO_EDOM;
		else if (!matherr(&exc)) {
		  if (_LIB_VERSION == _SVID_) {
		...
		if (_LIB_VERSION == _SVID_)
		  exc.retval = ( (x>kstandard_zero) ? POK_ERRNO_HUGE_VAL : -POK_ERRNO_HUGE_VAL);
		else
		  exc.retval = ( (x>kstandard_zero) ? POK_ERRNO_HUGE_VAL : -POK_ERRNO_HUGE_VAL);
		if (_LIB_VERSION == _POSIX_)
		  errno = POK_ERRNO_ERANGE;
		...
		if (_LIB_VERSION == _SVID_)
		  ...
		  exc.retval = kstandard_zero/kstandard_zero;
		if (_LIB_VERSION == _POSIX_)
		  errno = POK_ERRNO_EDOM;
		else if (!matherr(&exc)) {
		  if (_LIB_VERSION == _SVID_) {
	    ...
	    case 131:
                ...
                exc.type = SING;
                exc.name = type < 100 ? "atanh" : "atanhf";
		exc.retval = x/kstandard_zero;	/* sign(x)*inf */
                if (_LIB_VERSION == _POSIX_)
                  errno = POK_ERRNO_EDOM;
                else if (!matherr(&exc)) {
                  if (_LIB_VERSION == _SVID_) {
		...
		exc.type = UNDERFLOW;
		exc.name = type < 100 ? "scalb" : "scalbf";
		exc.retval = copysign(kstandard_zero,x);
		if (_LIB_VERSION == _POSIX_)
		  errno = POK_ERRNO_ERANGE;
		else if (!matherr(&exc)) {
			errno = POK_ERRNO_ERANGE;
